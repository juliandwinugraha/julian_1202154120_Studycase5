...
public class Database extends SQLiteOpenHelper {
    ...
    //method input data
    public boolean inputdata(MenambahData list) {
    ...
    public void readdata(ArrayList<MenambahData> daftar){
        Cursor cursor = this.getReadableDatabase().rawQuery("select todo, description, priority from "+nama_tabel, null);
        while (cursor.moveToNext()){
            daftar.add(new MenambahData(cursor.getString(0), cursor.getString(1), cursor.getString(2)));
        }
    }
}
